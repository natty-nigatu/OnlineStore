...
<body>
    ...
    <!-- ==== Checkout ++++ -->
    <form action="checkout.php" method="POST" class="detail-form detail-content">
        ...
        <div class="form-group">
            <h3>Total Price: &nbsp;&nbsp;<?php echo $total ?>ETB</h3>
        </div>

        <div class="form-group">
            <h3 <?php if ($total > $account->wallet) echo 'class="text-danger"'; ?>>Wallet:&nbsp;&nbsp;<?php echo $account->wallet ?>ETB</h3>
        </div>

        <?php if ($total > $account->wallet) { ?>

            <h4 class="text-danger">Not Enough Amount In Wallet To Buy All Items In Cart. </h4>

        <?php } else { ?>
            <button class="checkout-btn btn btn-success bg-green" type="submit" name="Checkout" value="Checkout">
                Checkout
            </button>
        <?php } ?>
    </form>
